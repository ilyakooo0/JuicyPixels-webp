...
spec = do
  describe "Golden Tests" $ do
    describe "Known WebP Files" $ do
      ...
      it "decodes test_webp_js.webp (VP8L lossless) correctly" $ do
        exists <- doesFileExist "test/data/test_webp_js.webp"
        if not exists
          then pendingWith "test_webp_js.webp not found"
          else do
            fileData <- B.readFile "test/data/test_webp_js.webp"
            case decodeWebP fileData of
              Right (ImageRGBA8 img) -> do
                -- Verify dimensions
                imageWidth img `shouldSatisfy` (> 0)
                imageHeight img `shouldSatisfy` (> 0)
                -- File should be lossless
                B.length fileData `shouldSatisfy` (> 1000000)
    ...
    describe "Specific Patterns" $ do
      it "handles checkerboard pattern" $ do
        let img = generateImage (\x y -> if even (x + y)
                                          then PixelRGB8 255 255 255
                                          else PixelRGB8 0 0 0) 64 64
      ...
      it "handles vertical stripes" $ do
        let img = generateImage (\x _ -> if even x
                                          then PixelRGB8 255 0 0
                                          else PixelRGB8 0 255 0) 64 64
            encoded = encodeWebPLossy img 90
      ...
      it "handles horizontal stripes" $ do
        let img = generateImage (\_ y -> if even y
                                          then PixelRGB8 255 0 0
                                          else PixelRGB8 0 0 255) 64 64
            encoded = encodeWebPLossy img 90
