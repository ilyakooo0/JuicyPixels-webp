...
spec = do
  describe "Property-Based Tests" $ do
    describe "Dimension Properties" $ do
      it "encodes and decodes arbitrary dimensions (small)" $ property $
      ...
      it "lossless preserves dimensions exactly" $ property $
        \(Positive w, Positive h) ->
          let w' = (w `mod` 128) + 1
              h' = (h `mod` 128) + 1
    ...
    describe "Color Properties" $ do
      it "solid color survives lossy encoding (approximate)" $ property $
        \r g b ->
           ...
           in case decodeWebP encoded of
                Right (ImageRGB8 dec) ->
                  let PixelRGB8 r' g' b' = pixelAt dec 16 16
                   ...
                   in abs (fromIntegral r' - fromIntegral r :: Int) < 40 &&
                      abs (fromIntegral g' - fromIntegral g :: Int) < 40 &&
                      abs (fromIntegral b' - fromIntegral b :: Int) < 40
                _ -> False

      it "lossless preserves exact color values" $ property $
        \r g b a ->
    ...
    describe "Encoding Stability" $ do
      it "encoding is deterministic (same input = same output)" $ property $
        \r g b ->
      ...
      it "lossless encoding is always deterministic" $ property $
        \r g b a ->
          let img = generateImage (\_ _ -> PixelRGBA8 r g b a) 16 16
    ...
    describe "Roundtrip Properties" $ do
      it "lossy encode-decode produces valid image" $ property $
      ...
      it "lossless roundtrip is identity" $ property $
        \r g b a ->
          let img = generateImage (\_ _ -> PixelRGBA8 r g b a) 16 16
              encoded = encodeWebPLossless img
           in case decodeWebP encoded of
                Right (ImageRGBA8 dec) ->
                  all (\(x, y) -> pixelAt dec x y == PixelRGBA8 r g b a)
                      [(x, y) | x <- [0..15], y <- [0..15]]
                _ -> False

    describe "File Format Properties" $ do
      it "all encoded files start with RIFF signature" $ property $
        \r g b ->
          let img = generateImage (\_ _ -> PixelRGB8 r g b) 16 16
              encoded = encodeWebPLossy img 80
      ...
      it "all encoded files have WEBP signature" $ property $
        \r g b ->
          let img = generateImage (\_ _ -> PixelRGB8 r g b) 16 16
              encoded = encodeWebPLossy img 80
           in B.length encoded >= 12 &&
      ...
      it "file size is reasonable for dimensions" $ property $
        \(Positive w, Positive h) ->
          let w' = (w `mod` 64) + 1
              h' = (h `mod` 64) + 1
              img = generateImage (\_ _ -> PixelRGB8 128 128 128) w' h'
